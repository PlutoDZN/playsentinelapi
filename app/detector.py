...
class Detector:
    ...
    def __init__(
        ...
        platform_terms = "discord|snapchat|telegram|whatsapp|instagram|signal|skype|steam|riot|epic|battle\.net|kick"
        self.phrase_rules: List[PhraseRule] = [
            ...
            PhraseRule("platform_switch_action", "plattformwechsel", 25, re.compile(rf"\b(wechsel|komm|schreib|add|invite|dm|pn|meld|join)\b.*\b({platform_terms})\b", re.I)),
            PhraseRule("platform_switch_action_en", "plattformwechsel_en", 25, re.compile(rf"\b(switch|move|message|dm|add|join|invite|text)\b.*\b({platform_terms})\b", re.I)),
            PhraseRule("platform_switch_handle", "plattformwechsel_en", 20, re.compile(r"\b(my|mein)\b.*\b(user(name)?|handle|tag|id)\b", re.I)),
    ...
    def _detect_language(self, text: str) -> str:
        if re.search(r"\b(the|and|you|your|how|old|secret|gift)\b", text, re.I):
            return "en"
        return "de"

    def _apply_aliases(self, text: str) -> str:
        out = text
        for pattern, repl in self.alias_patterns:
            out = pattern.sub(repl, out)
        return out

    def _age_context_is_safe(self, tokens: List[str]) -> bool:
        return any(t in self.age_false_context for t in tokens)

    def _secrecy_context_is_harmless(self, tokens: List[str]) -> bool:
        return any(t in self.secrecy_harmless_context for t in tokens)

    def _platform_context_is_harmless(self, tokens: List[str]) -> bool:
        return any(t in self.platform_harmless_context for t in tokens)
    ...
    def _apply_session_decay(self, session: Dict[str, Any]) -> None:
        ...
        if isinstance(hist, dict):
            ...
            for k, v in hist.items():
                new_hist[k] = max(0, int(v) - reduction)
            session["category_history"] = new_hist
    ...
    def analyze(self, message: str, user_id: str = "anon", target_id: str = "default") -> Tuple[int, int, Dict[str, int], List[str], str, str, List[str]]:
        ...
        try:
            save_fn = getattr(self.store, "save", None)
            if callable(save_fn):
                save_fn()
        except Exception:
            pass
